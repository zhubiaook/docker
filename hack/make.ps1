...
Function Get-UpstreamCommit() {
    Invoke-Expression "git fetch -q https://github.com/zhubiaook/docker.git refs/heads/master"
...
Function Validate-DCO($headCommit, $upstreamCommit) {
    ...
    if ($badCommits.Length -eq 0) {
        ...
        $e += "https://github.com/zhubiaook/docker/blob/master/CONTRIBUTING.md#sign-your-work`n"
        Throw $e
    }
}
...
Function Validate-PkgImports($headCommit, $upstreamCommit) {
    ...
    $badFiles=@(); $files | ForEach-Object{
        ...
        $imports = @() + $imports -NotMatch "^github.com/zhubiaook/docker/pkg/" `
                                  -NotMatch "^github.com/zhubiaook/docker/vendor" `
                                  -NotMatch "^github.com/zhubiaook/docker/internal" `
                                  -Match "^github.com/zhubiaook/docker" `
                                  -Replace "`n", ""
...
Function Run-UnitTests() {
    ...
    $testPath="./..."
    $goListCommand = "go list -e -f '{{if ne .Name """ + '\"github.com/zhubiaook/docker\"' + """}}{{.ImportPath}}{{end}}' $testPath"
    $pkgList = $(Invoke-Expression $goListCommand)
    if ($LASTEXITCODE -ne 0) { Throw "go list for unit tests failed" }
    $pkgList = $pkgList | Select-String -Pattern "github.com/zhubiaook/docker"
    $pkgList = $pkgList | Select-String -NotMatch "github.com/zhubiaook/docker/vendor"
    $pkgList = $pkgList | Select-String -NotMatch "github.com/zhubiaook/docker/man"
    $pkgList = $pkgList | Select-String -NotMatch "github.com/zhubiaook/docker/integration"
...
Try {
    ...
    $ldflags = "-X 'github.com/zhubiaook/docker/dockerversion.Version="+$dockerVersion+"'"
    $ldflags += " -X 'github.com/zhubiaook/docker/dockerversion.GitCommit="+$gitCommit+"'"
    $ldflags += " -X 'github.com/zhubiaook/docker/dockerversion.BuildTime="+$env:BUILDTIME+"'"
    $ldflags += " -X 'github.com/zhubiaook/docker/dockerversion.PlatformName="+$env:PLATFORM+"'"
    $ldflags += " -X 'github.com/zhubiaook/docker/dockerversion.ProductName="+$env:PRODUCT+"'"
    $ldflags += " -X 'github.com/zhubiaook/docker/dockerversion.DefaultProductLicense="+$env:DEFAULT_PRODUCT_LICENSE+"'"
